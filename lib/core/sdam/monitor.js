...
class Monitor extends EventEmitter {
  constructor(server, options) {
    ...
    const connectOptions = Object.assign(
      {
        id: `<monitor ${server.description.host}:${server.description.port}>`,
...
function checkServer(monitor, callback) {
  ...
  function failureHandler(err) {
    ...
    if (err instanceof MongoNetworkError && monitor[kMonitorId]) {
  ...
  if (monitor[kConnection] != null && !monitor[kConnection].closed) {
    ...
    monitor[kConnection].command('admin.$cmd', cmd, options, (err, result) => {
      if (err) return failureHandler(err);
...
function monitorServer(monitor) {
  return function monitoring(callback) {
    ...
    checkServer(monitor, function checkedServer(err, isMaster) {
      if (err) {
